...
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 여기 위쪽은 mobile이지만 뺵업 느낌으로 놔둠

# 12-27 JJS
def userManageHtml(request):
    try:
        userinfo = SignUp.objects.all()  # == 모든정보를 가져와
        # userinfo = SignUp.objects.filter(name = "정정수") == 다수의 원하는 정보
        # userinfo = SignUp.objects.get(id = 35)
        context = {"userinfo" : userinfo}
        return render(request, 'app/userManage.html', context)
    except Exception as error:
    	print("userManageHtml", error)

def userDetailHtml(request):
    try:
        userID = request.POST.get('userID')
        print("userID ::", userID)
        userinfo = SignUp.objects.get(id = userID)
        context = {"userinfo" : userinfo}
        return render(request, 'app/userManageDetail.html', context)
    except Exception as error:
    	print("userDetailHtml", error)



# 2021.12.28 HSW 작성 - Admin -> 멤버관리 -> 작가관리 -----
def authorManageHtml(request):
    try:
        authorinfo = SignUp.objects.all()
        context = {"authorinfo" : authorinfo}
        return render(request, 'app2/authorManage.html', context)
    except Exception as error:
    	print("authorManageHtml", error)

def authorDetailHtml(request):
    try:
        userID = request.POST.get('userID')
        print("userID ::", userID)
        userinfo = SignUp.objects.get(id = userID)
        context = {"userinfo" : userinfo}
        return render(request, 'app2/authorManageDetail.html', context)
    except Exception as error:
    	print("userDetailHtml", error)
